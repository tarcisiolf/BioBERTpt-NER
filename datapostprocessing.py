...
def create_dataframe_with_result_ner_format(df_result):
    #df_result = read_csv(df_path)

    for col in ['Token', 'IOB_label', 'IOB_label_pred']:
        df_result[col] = df_result[col].apply(eval)

    df_concat = df_result.groupby('Report').agg({
        'Token': lambda x: sum(x, []),
        'IOB_label': lambda x: sum(x, []),
        'IOB_label_pred': lambda x: sum(x, [])
    }).reset_index()

    df_ner_format = df_concat.explode(['Token', 'IOB_label', 'IOB_label_pred']).reset_index(drop=True)
    df_ner_format.insert(0, 'index', range(1, 1 + len(df_ner_format)))
    #df_ner_format.to_csv('data/results_ner_format.csv', encoding='utf-8', index=False)

    return df_ner_format
...
def extract_info_from_report(report_df):
    ...
    for _, row in report_df.iterrows():
        ...
        if tag_pred == 'B-ACH':
            ...
            if lower_Token in atenuacoes_solido:
                nodule_data['solid'] = "Sim"
            elif lower_Token in atenuacoes_semissolido:
                nodule_data['semisolid'] = "Sim"
            elif lower_Token in atenuacoes_vidro_fosco:
...
if __name__ == '__main__':
    #df_result_path = 'data/results.csv'
    #df_result = read_csv(df_result_path)          

    df_result = pd.read_csv('data/results.csv')
    df_ner_format = create_dataframe_with_result_ner_format(df_result)
    ...
    df_results_post_processed = create_dataframe_post_processed(df_ner_format, nodule_info)
